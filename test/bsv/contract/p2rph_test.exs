defmodule BSV.Contract.P2RPHTest do
  ...
  describe "Contract.test_run/3" do
    ...
    test "evaluates as valid if signed with correct key", ctx do
      assert {:ok, vm} = Contract.test_run(P2RPH, %{r: ctx.r}, %{k: ctx.k, keypair: KeyPair.new()})
      assert VM.valid?(vm)
    end

    test "evaluates as invalid if used with with incorrect k", ctx do
      assert {:error, vm} = Contract.test_run(P2RPH, %{r: ctx.r}, %{k: P2RPH.generate_k(), keypair: KeyPair.new()})
      refute VM.valid?(vm)
    end
  end
